    ...
    def _process_input_core(self, user_input: str, session_id: str) -> str:
        ...
        for attempt, model in enumerate([self.primary_model] + self.fallback_models):
            try:
                ...
                CORE_ROUTER_TOOLS = SYSTEM_TOOLS + MEMORY_TOOLS + VISION_TOOLS
                for tool in CORE_ROUTER_TOOLS:
                    if tool.__name__ in router_response:
                        logger.log(f"Textual tool call pattern detected for '{tool.__name__}'. Executing core tool...", category="TOOL")
                        ...
                        if match:
                            ...
                            if args_str:
                                ...
                                try:
                                    ...
                                    logger.log(f"Failed to parse args for core tool '{tool.__name__}': {parse_err}", category="SYSTEM")
                        
                        try:
                            result = tool(*args, **kwargs)
                        except Exception as e:
                            result = f"Error executing tool '{tool.__name__}': {str(e)}"
                            
                        logger.log(f"Core tool '{tool.__name__}' returned: {result}", category="TOOL")
